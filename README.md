...
- � **多学生账户** — 家庭多孩场景下按学生隔离错题库，一人一库互不干扰，支持全家 AI 用量合计
- �📊 **学习数据** — 练习次数、掌握程度、难度评分全记录
- 💾 **本地存储** — 所有数据存本地 SQLite，图片保留原位置- 📚 **学科知识库问答** — 对接 [sida-agent](https://github.com/wubo0067/sida-agent)，教材 PDF 建库后可多轮问答、答案溯源教材页码并附原图（详见[「知识库对接」](#-知识库对接sida-agent)）
## 功能概览
...
适合家庭有多个学生的场景：每个学生拥有**完全独立**的错题库、重点练、图片目录配置与 AI Token 统计，数据按人分库存放、互不可见。

- **🎒 学生切换器** — 页面顶部标题旁下拉框一键切换当前学生，切换后所有页面数据自动重载
- **🗂️ 一人一库** — 每个学生对应独立的 SQLite 库文件（`server/data_s<id>.db`），错题、标签、解答、练习记录完全隔离
- **⚙️ 独立配置** — 图片目录、重点练超时阈值等配置按学生独立保存，各学生可指向各自的图片文件夹
- **➕ 学生管理** — 配置页支持添加、重命名、删除学生；删除会一并清除该学生的专属库（图片文件保留），最后一个学生不可删除
- **📊 全家合计** — AI Token 统计支持「当前学生 / 全家合计」两种视图，全家视图展示所有学生的消耗合计及各学生分项
- **🔒 零迁移兼容** — 升级后原有 `data.db` 自动登记为「默认」学生，历史数据无需任何迁移
- **🌐 多设备共享** — 学生身份随请求自动携带（请求头 / 查询参数），多台设备访问同一后端时各自记住当前学生

> ⚠️ 当前为家庭内共享模式，切换学生无需密码，请在可信的局域网环境中使用。

## 📚 知识库对接（sida-agent）

除了管理「学生自己拍下来的错题」，本应用还内置了一个**学科知识库**页签，对接独立的
开源项目 **[wubo0067/sida-agent：中学理科全科知识库问答 Agent](https://github.com/wubo0067/sida-agent)**。
sida-agent 把初中**物理 / 化学 / 数学**教材 / 讲义 PDF 用视觉大模型提取、再用推理大模型
结构化抽取，沉淀为「知识图谱 + 向量库」双库，能对学生提问生成**可溯源到教材页码**的分层讲解。

错题本负责「练」（整理、复习自己的错题），sida-agent 负责「学」（讲解知识点、查公式、看教材原图），
两者互补：遇到不会的知识点，直接在错题本里问知识库，答案能贴出对应教材原图。

### 功能特点

- **💬 多轮对话问答** — 知识库页的「对话」子页可与 sida-agent 连续追问，会话按 `thread_id` 持久化，历史会话可回看续聊；回答经 SSE 逐字流式输出，并可折叠查看模型的「思考过程」。
- **🧮 公式与来源标注** — 回答中的 LaTeX 公式（`$...$` / `$$...$$`）由 KaTeX 渲染，讲解末尾标注「（见《教材名》第 X 页）」，可溯源到具体教材页码。
- **🖼️ 教材原图缩略图行** — 回答关联的教材整页原图会以**一行缩略图**展示在聊天最末，点击任意图即可**放大原图并左右翻页**（箭头按钮 / 键盘 ← / →，支持滚轮缩放、拖拽、1:1），电路图 / 几何图 / 实验装置图不再只靠文字转写。
- **📂 教材清单** — 「教材」子页展示已导入知识库的所有教材（名称 + `pdf_id`）。
- **📥 教材导入（建库）** — 「导入」子页点「选择文件」逐级浏览本机目录挑出教材 PDF（也可直接粘贴绝对路径），再填页码区间与学科即可建库；提交前先做**只读「规模预估」**（0 模型调用，统计将新增多少次视觉 / 推理调用），确认后才真正跑；建库经 SSE 实时展示逐页 / 逐子块进度与真实 token 消耗，支持断点续跑与任务历史回看。
- **🔗 同源代理、零跨域** — 前端统一请求本应用后端的 `/api/agent/*`，由后端用 httpx 转发到 sida-agent 服务，改配置即时生效、不依赖上游 CORS。

### 如何配置

1. **启动 sida-agent 服务**（独立项目，需单独准备其 Python 环境）。在 `sida-agent` 目录下执行：

   ```bash
   uv run python main.py --stage serve --host 127.0.0.1 --port 6173
   ```

   > 若尚未建库，可先用 `--stage build` 灌入教材 PDF，或直接在本应用「知识库 → 导入」子页操作。
   > sida-agent 自身的安装、`.env`（视觉 / 推理模型 Key）、建库细节请见其
   > [项目 README](https://github.com/wubo0067/sida-agent)。

2. **在本应用中填写服务地址**：进入 **配置&统计** 页面 → **知识库服务（sida-agent）** 区块，填写：

   | 字段        | 默认值          | 说明                                              |
   | --------- | ------------ | ----------------------------------------------- |
   | 服务地址 Host | `127.0.0.1`  | sida-agent 监听地址；局域网访问可填 `http://192.168.1.10` 等 |
   | 端口 Port   | `6173`       | sida-agent `--port` 指定的端口                      |

   点击 **保存配置**（保存后会自动测连），或点 **测试连接** 手动验证。连接成功会显示
   「✅ 连接成功 · 图谱 N 节点 / 向量 M 条」。

   > 📌 该配置为**全局配置**（存于 `server/students.db` 的 `global_config` 表），对所有学生共享、
   > 修改后即时生效，无需重启后端。

3. **开始使用**：顶部切换到 **知识库** 页签，在「对话」子页新建会话并向知识库提问，或在「导入」子页灌入新教材。

### 代理接口一览

本应用后端（`server/server.py`）将以下 `/api/agent/*` 端点转发到 sida-agent 的对应 HTTP API：
...
| POST   | `/api/agent/build/estimate`              | 建库规模预估（只读、0 模型调用）               |
| POST   | `/api/agent/build`                       | 提交建库任务（存在新调用时 409 返回预估待确认）      |
| GET    | `/api/agent/build/tasks`                 | 建库任务历史                          |
| GET    | `/api/agent/build/tasks/{id}`            | 单个建库任务状态                        |
| GET    | `/api/agent/build/tasks/{id}/events`     | 建库进度 SSE（逐页 / 逐子块）              |

> 📂 另有两个**本机文件浏览**接口（不经过 sida-agent）：`GET /api/fs/roots` 列出可用盘符 / 主目录，
> `GET /api/fs/list?path=<绝对路径>` 列出该目录下的子目录与 PDF 文件。导入页的「选择文件」控件靠它们
> 弥补浏览器无法获取本地文件绝对路径的限制；仅返回名称 / 路径 / 大小，不读取文件内容，适合本地或可信局域网使用。

> ⚠️ **常见问题**：知识库页显示「无法连接 sida-agent 知识库服务」时，请先确认步骤 1 的服务已启动、
> Host / Port 与实际监听一致，再点「测试连接」重试。教材原图缩略图加载不出来，通常是
> sida-agent 未开启 `--stage serve` 的静态图片挂载（`/pdf_images`），或其 `.env` 未配置对外可访问的图片基址。
...
- **多学生路由**：`students.db` 只存学生名单；每个请求由中间件按 `X-Student-Id` 头（或 `?sid=` 参数）路由到对应学生的 SQLite 库，并发请求互不串库
...
目录配置保存在后端 SQLite 数据库中，重启后依然生效。**多学生模式下，该配置按学生独立保存**，不同学生可指向各自的图片目录。
...
└── server/
    ...
    ├── data.db                 # 默认学生的 SQLite 数据库（自动创建，历史数据零迁移）
    ├── students.db             # 学生注册表（多账户名单，自动创建）
    ├── data_s2.db ...          # 其他学生的专属库（新建学生时自动创建）
    └── logs/
...
| GET    | `/api/image-file`             | 提供图片文件访问服务（参数：`path`，多学生模式下可附 `sid`）                    |
| GET    | `/api/students`               | 获取学生列表与当前学生 id                                            |
| POST   | `/api/students`               | 新建学生（参数：`name`），自动创建其专属库                             |
| PUT    | `/api/students/{id}`          | 重命名学生（参数：`name`）                                          |
| DELETE | `/api/students/{id}`          | 删除学生及其专属库（图片文件保留；最后一个学生不可删除）                        |
| GET    | `/api/token-stats`            | AI Token 消耗统计（参数：`scope=student` 当前学生 / `all` 全家合计+各学生分项） |

> 👥 **多学生请求头**：除 `/api/students` 外，所有数据类接口都按请求头 `X-Student-Id` 路由到对应学生的库；`<img>` 等无法带头的场景可用 `?sid=` 查询参数。未携带时回退到第一个学生（默认库）。
...
1. 进入 **配置&统计** 页面，在 **学生管理** 区块输入姓名点击 **添加学生**，新学生创建后会自动切换过去
2. 为当前学生设置其专属的 **图片目录路径** 并保存（各学生可指向不同文件夹，如 `D:\小明\错题`、`E:\小红\错题`）
3. 日常使用时，通过页面顶部的 **学生切换器** 下拉框切换当前学生，切换后错题库、重点练、时间线、统计等数据自动重载为该学生的
4. 重命名：在学生列表中点击 ✎ 按钮行内编辑，回车确认
5. 删除：点击 🗑 按钮并确认，会一并删除该学生的错题数据（图片文件保留在磁盘上）；至少需保留一个学生
6. 在 **AI Token 统计** 区块点击「全家合计」，可查看全家消耗总量及各学生分项
...
> A: 单学生时数据在 `server/data.db`；多学生模式下每个学生有独立库（`server/data_s<id>.db`），另有学生名单注册表 `server/students.db`。备份时请复制 `server/` 下全部 `*.db` 文件。
...
> A: 删除 `server/` 下的全部 `*.db` 文件（含 `students.db` 与各学生库）并重启后端，系统会创建新的空数据库与默认学生。也可在配置页的「学生管理」中单独删除某个学生。

**Q: 多学生之间会看到彼此的错题吗？**

> A: 不会。每个学生的错题、重点练、解答、AI 统计都存在各自的库文件中，后端按请求携带的学生身份路由到对应库，跨库不可见。仅「全家合计」的 AI Token 统计会展示所有学生的消耗汇总。

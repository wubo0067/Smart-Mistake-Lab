...
MATH_KNOWLEDGE_POINTS = [
    ...
    "全等三角形判定 1：SSS（边边边）",
    "全等三角形判定 2：SAS（边角边，注意角必须是两边的夹角）",
    "全等三角形判定 3：ASA（角边角）",
    "全等三角形判定 4：AAS（角角边）",
    "全等三角形判定 5：HL（斜边、直角边，仅限直角三角形）",

    # 2. 经典基础模型
    "全等基础模型：平移模型（图形沿某方向平移，对应边平行且相等，常隐含同位角/内错角相等）",
    "全等基础模型：轴对称/翻折模型（沿某直线翻折，对称轴垂直平分对应点连线，对应角/边相等）",
    "全等基础模型：旋转模型（绕某点旋转，对应点到旋转中心距离相等，夹角等于旋转角）",
    "全等基础模型：公共边模型（隐含条件，SSS/SAS/ASA 常用，如两个三角形拼在一起）",
    "全等基础模型：公共角/对顶角模型（隐含等角条件，ASA/AAS 常用）",
    "全等基础模型：角平分线 + 垂线模型（角平分线上的点到角两边距离相等，构造直角三角形全等）",
    "全等基础模型：角平分线 + 平行线模型（一角平分线 + 平行线=等腰三角形，转移线段）",

    # 3. 压轴高阶模型（中考必考）
    "全等高阶模型：手拉手模型（共顶点的两个等边/等腰直角三角形旋转，必产生一对新的全等三角形）",
    "全等高阶模型：截长补短模型（证明线段和差关系的核心辅助线，在长线段截取或延长短线段构造全等）",
    "全等高阶模型：半角模型（正方形或等腰直角三角形内部含一半角，通过旋转或截长补短构造全等）",
    "全等高阶模型：垂直平分线模型（线段垂直平分线上的点到两端点距离相等，常结合等腰三角形）",
    "全等高阶模型：中线与高线重合模型（等腰三角形三线合一的逆定理，遇此条件必出全等）",
    "全等高阶模型：婆罗摩笈多模型（双等腰直角三角形共顶点，若取底边中点连线，有垂直且相等的结论）",
    "全等高阶模型：对角互补模型（共顶点的两个三角形对角互补，常通过作双垂线构造全等转移线段）",
    "全等高阶模型：角含半角模型（大角包含一个小角，且小角是大角的一半，常通过翻折构造全等）",
